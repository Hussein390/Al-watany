...
import { CreateDeliveryTask, DataPhones } from '../components/DataProvider';
import { GET_COLLABORATORS, UPDATE_DELIVERY_TASK } from '@/backend/delivery';
import { supabase } from '@/supabase';
import { EllipsisVertical } from 'lucide-react';
import TablesCURD from '../components/TablesCURD';
...
export default function Tables() {
  ...
  const [date, setDate] = React.useState<number>(new Date().getDate());
  ...
  const filteredTasks = tasks.filter(task => {
    if (!task.isReceived) return false;
  ...
  return (
    <div className='w-[1300px] mx-auto mt-12 mb-2'>
      <div className="flex justify-between items-start ">
        <div className="flex items-center gap-x-4">
          ...
          <Select value={date?.toString()} onValueChange={(value) => setDate(parseInt(value))} >
            <SelectTrigger className="w-[140px] cursor-pointer font-semibold outline-none">
              <SelectValue placeholder="Theme" />
            </SelectTrigger>
            <SelectContent className='z-40 bg-white max-h-[300px]'>
              <SelectGroup>
                <SelectLabel className='border-b font-semibold'>اليوم</SelectLabel>
                {Array.from({ length: daysOfMonth }, (_, i) => i + 1).map((day) => (
                  <SelectItem className='cursor-pointer hover:bg-stone-200 delay-100' key={day} value={day.toString()}>
                    {day}
                  </SelectItem>
                ))}
              </SelectGroup>
      ...
      <Link href={'/'} className='py-2 px-4 rounded bg-blue-500 text-white hover:bg-blue-700 delay-100 fixed bottom-7 right-7'>Back</Link>
    </div>
